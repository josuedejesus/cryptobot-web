...
const SYMBOLS = ['XRPUSDT', 'BTCUSDT', 'ETHUSDT', 'DOGEUSDT', 'BNBUSDT', 'SOLUSDT'];
const TIMEFRAMES = ['3m', '5m', '15m'];

export default function Home() {
  const { summary, lastSignal, connected, config, updateConfig, applyPreset, closeActiveTrade } = useBot();
  ...
  const trendColor = (t: string) =>
    t === 'BULLISH' ? 'text-emerald-400' : t === 'BEARISH' ? 'text-red-400' : 'text-gray-500';
  ...
  if (lastSignal && summary?.activeTrade) {
    ...
    const pc = summary.activeTrade.type === 'LONG' ? (cp - ep) / ep : (ep - cp) / ep;
    unrealizedPnl = ps * pc;
    progressPct = Math.min(Math.abs(pc / (config?.takeProfitPercent ?? 0.01)) * 100, 100);
  }
      ...
      <div className="px-6 py-6 max-w-5xl mx-auto space-y-5">
        ...
        {tab === 'dashboard' && (
          <>
            {/* Signal card */}
            {lastSignal && (
              <div className={`rounded-xl border p-5 transition-colors ${signalBg[lastSignal.signal]}`}>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-widest mb-1">Última señal</p>
                    <div className="flex items-center gap-2">
                      <span className={`text-3xl font-bold ${signalTextColor[lastSignal.signal]}`}>
                        {lastSignal.signal}
                      </span>
                    ...
                    {lastSignal.reason && lastSignal.signal !== 'HOLD' && (
                      <p className="text-xs text-gray-400 mt-1">{lastSignal.reason}</p>
                    )}
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-gray-500">Precio</p>
                    <p className="text-xl font-mono font-semibold">${lastSignal.price.toFixed(4)}</p>
                  </div>
                </div>

                <div className="grid grid-cols-4 md:grid-cols-7 gap-3 pt-4 border-t border-gray-700/50 text-xs">
                  {[
                    { label: `EMA ${config?.emaFast ?? 9}`, value: lastSignal.ema9.toFixed(4) },
                    { label: `EMA ${config?.emaSlow ?? 21}`, value: lastSignal.ema21.toFixed(4) },
                    {
                      label: 'RSI',
                      value: lastSignal.rsi.toFixed(1),
                      color: lastSignal.rsi > 70 ? 'text-red-400' : lastSignal.rsi < 30 ? 'text-emerald-400' : 'text-white',
                    },
                    {
                      label: 'VWAP',
                      value: lastSignal.priceAboveVwap ? '↑ Sobre' : '↓ Bajo',
                      color: lastSignal.priceAboveVwap ? 'text-emerald-400' : 'text-red-400',
                    },
                    {
                      label: 'Volumen',
                      value: lastSignal.isHighVolume ? 'Alto' : 'Bajo',
                      color: lastSignal.isHighVolume ? 'text-emerald-400' : 'text-gray-500',
                    },
                    {
                      label: 'BB Width',
                      value: `${lastSignal.bbWidth?.toFixed(2)}%`,
                      color: lastSignal.bbSqueeze ? 'text-amber-400' : 'text-white',
                    },
                    {
                    ...
                    <div key={label}>
                      ...
                      <p className={`font-medium ${color ?? 'text-white'}`}>{value}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {summary && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  { label: 'Balance', value: `$${summary.balance.toFixed(2)}`, color: 'text-white' },
                  {
                    label: 'PnL Total',
                    value: `${parseFloat(summary.totalPnl) >= 0 ? '+' : ''}$${summary.totalPnl}`,
                    color: parseFloat(summary.totalPnl) >= 0 ? 'text-emerald-400' : 'text-red-400',
                  },
                  { label: 'Win Rate', value: summary.winRate, color: 'text-white' },
                  {
                    label: 'Trades',
            ...
            {summary?.activeTrade && (
              <div className="bg-gray-900 border border-amber-500/30 rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-xs font-semibold text-amber-400 uppercase tracking-widest">Trade activo</p>
                  <button
                    onClick={async () => {
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm mb-4">
                  <div>
                    ...
                    <p className={`font-bold text-lg ${summary.activeTrade.type === 'LONG' ? 'text-emerald-400' : 'text-red-400'}`}>
                      {summary.activeTrade.type}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 mb-0.5">Entrada</p>
                    <p className="font-mono font-medium">${summary.activeTrade.entryPrice.toFixed(4)}</p>
                  </div>
                  {unrealizedPnl !== null && (
                    <div>
                      <p className="text-xs text-gray-500 mb-0.5">PnL actual</p>
                      <p className={`font-bold text-lg ${unrealizedPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  ...
                  <div>
                    ...
                    <p className="font-mono text-red-400">${summary.activeTrade.stopLoss.toFixed(4)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 mb-0.5">Take Profit</p>
                    <p className="font-mono text-emerald-400">${summary.activeTrade.takeProfit.toFixed(4)}</p>
                  </div>
                  {lastSignal && (
                    <div>
                      <p className="text-xs text-gray-500 mb-0.5">Precio actual</p>
                      <p className="font-mono font-medium">${lastSignal.price.toFixed(4)}</p>
                    </div>
                  )}
                </div>
            ...
            {summary && summary.trades.filter(t => t.result).length > 0 && (
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                <p className="text-xs text-gray-500 uppercase tracking-widest mb-4">Historial</p>
                <div className="space-y-1">
                  {summary.trades.filter(t => t.result).map(trade => (
                    <div
                      key={trade.id}
                    ...
                    >
                      ...
                      <span className={`font-bold ${trade.result === 'WIN' ? 'text-emerald-400' : 'text-red-400'}`}>
        ...
        {tab === 'config' && config && (
          <>
            ...
            {/* Symbol */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-3">Par de trading</p>
            ...
            {/* Timeframe */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-3">Timeframe</p>
              ...
              <p className="text-gray-600 text-xs mt-2">Cambiar el timeframe aplica el preset automáticamente.</p>
            </div>

            {/* Risk */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-4">Gestión de riesgo</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { label: 'Position Size (USDT)', key: 'positionSize', type: 'float' },
                  { label: 'Stop Loss %', key: 'stopLossPercent', type: 'float', step: 0.001 },
                  { label: 'Take Profit %', key: 'takeProfitPercent', type: 'float', step: 0.001 },
                  ...
                  <div key={key}>
                    ...
                    <input
                      ...
                      onChange={e => handleConfigChange(key, type === 'float' ? parseFloat(e.target.value) : parseInt(e.target.value))}
            ...
            {/* Indicators */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-4">Indicadores</p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  { label: 'Max Trades/día', key: 'maxTradesPerDay', type: 'int' },
                  { label: 'Min Band Width %', key: 'minBandWidth', type: 'float', step: 0.1 },
                ].map(({ label, key, type, step }) => (
                  <div key={key}>
                    <label className="text-xs text-gray-500 block mb-1">{label}</label>
                    <input
            ...
            {/* Toggles */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-4">Opciones</p>
              <div className="space-y-4">
                {[
                  { key: 'vwapStrict', label: 'VWAP estricto', desc: 'Solo opera en la dirección del VWAP del día' },
                  { key: 'onlyLong', label: 'Solo LONG', desc: 'Ignora todas las señales SHORT' },
                  { key: 'onlyShort', label: 'Solo SHORT', desc: 'Ignora todas las señales LONG' },
                ].map(({ key, label, desc }) => {
